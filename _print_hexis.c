#include "main.h"
/**
 * print_hexade - Prints a hexadecimal number in lower or upper case
 * @arg_list: A list of arguments containing the unsigned number to be printed
 * @map_to: Array of values to map the number to (either "0123456789abcdef" or "0123456789ABCDEF")
 * @output_buffer: Buffer array to handle print
 * @format_flags: Calculates active flags for formatting
 * @flag_char: The character to represent the 'hash' flag('x' for lower case, 'X' for upper case)
...
*/
int print_hexade(va_list arg_list, char mah_to[], char output_buffer[], int format_flags, char flag_char, int field_width, int precision_val, int length_mod)
{
int i = BUFF_SIZE - 2;
...
i++;
return (write_unsgnd(0, i, output_buffer, format_flags, field_width, precision_val, length_mod));
}
